...
@push('judul', 'Kelola Wali Kelas')
@push('breadcrumb', 'Kelola Wali Kelas')

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ $judulcrud }}</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <a type="button" href="{{ route('kelola.wali_kelas.tambah') }}" class="btn btn-primary shadow-md mr-2">Tambah
                Wali Kelas</a>
        </div>
    </div>
...
{{--                <form name="cari" id="cari" method="get" action="{{ route('kelola.siswa.cari') }}" class="xl:flex sm:mr-auto">--}}
...
{{--            </div>--}}
            <table id="table" class="table table-report yajra-datatable -mt-2">
                ...
                @foreach($data_wali_kelas as $list)
                    <tr class="intro-x">
                        ...
                        <td class="table-report__action w-56">
                            <div class="flex justify-center items-center">
                                <a class="flex items-center mr-3" href="{{ Request::url() }}/edit/{{ $list->id }}">
                                    <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit
                                </a>
                                <a class="flex items-center text-theme-21"
                                   href="{{ Request::url() }}/hapus/{{ $list->user->id }}"
